...
import 'package:real_time_sign_lansuage_v2/data/services/base_repository.dart'
    show BaseRepository;
...
class ChatRepository extends BaseRepository {
  ...
  Future<void> updateOnlineStatus(String documentId, bool isOnline) async {
    try {
      DocumentReference docRef = firestore.collection('users').doc(documentId);
      DocumentSnapshot docSnapshot = await docRef.get();
      if (docSnapshot.exists) {
        await docRef.update({'online': isOnline});
      } else {
        // Handle the case where the document does not exist.
        // You could create the document here if that fits your application's logic.
      }
    } catch (e) {
      rethrow;
    }
